...
|---|---|---|---|---|
| A0.1 governance | 验证治理工件 | `track_registry_v1`、`walk_forward_calendar_S1M_v1`、`WalkForwardCalendarValidator`、`universe_daily_construction_audit`、valuation coverage audit | 预计 2-3 周；由交易日历、feature/label panel hash 和 `validation_params.json` 生成；校验 computed purge、embargo、holdout 隔离、label maturity、窗口重叠率和参数 hash；估值缺口对 holdout 同时报告 drop-gap 与 ffill 两口径，ffill 只能有界用于慢变量；估值字段 PIT 用随机 50 只股票对比财报公告日与估值更新日 | 阻塞官方 S1 启动 |
| A0.2 keep governance / execution audit | 可执行收益审计和台账 | `holdout_access_log.tsv`、`test_family_registry`、冻结模型 registry、SQLite WAL `experiment_ledger`、`execution_label_audit`、`execution_audit/orders_audit`、`daily_turnover_capacity_report` | 预计 3-6 周；用未复权 OHLCV、`tradability_daily_enriched`、`universe_daily`、成本表和 T+1/分批执行规则生成 open/proxy、3/5 日分批、未成交 carryover、解锁反转和订单状态；orders audit 至少含 `execution_price`、`execution_slippage`、`order_status`；日频 L1 square-root impact model 至少使用成交额、参与率和波动率或替代波动代理；holdout log 记录污染状态，burned holdout 后需要不少于 252 个交易日 shadow/forward OOS | 阻塞 factor keep、S1-D/S1-R tighten-only 和任何日频 alpha 声明 |
...
| P1.5 GMSL | 外生冲击完整面板 | GMSL-v1 partial stress report；GMSL-v2 FX、rates、spot/futures energy、global equity futures、MOVE、commodity basket、地缘事件日历 | v1 只使用 Cboe VIX/OVX/GVZ、国债收益率和 Shibor 的 candidate 数据输出 stress report；v2 为 FRED 超时源准备 fredapi、Nasdaq Data Link/Quandl、vendor mirror 或下载替代；全部转 UTC 后按 A 股 T 日 16:00 session cutoff 顺延；阈值只用训练窗 10 年 lookback 分位数计算 | GMSL 继续只能 partial stress report，不能进入 alpha、选模或调阈值 |
