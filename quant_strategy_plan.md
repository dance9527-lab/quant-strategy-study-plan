...
|---|---|---|---|---|
| P1 | 日频多因子强基线 | 直接可做 | 证明 warehouse 下可交易超额是否存在 | 立即启动 |
| P1 | 交易可行性和容量压力测试 | 直接可做 | 量化真实成交边界和成本拖累 | 立即启动 |
| P1 | 市场状态和风险开关 | 直接可做 | 降低回撤和波动 | 并行研究 |
...
| 排名 | 数据源 | 价值 | 裁决 |
|---:|---|---|---|
| 1 | 公司行为、分红、送配 | total return、分红因子、复权校验 | P2 优先接入 |
| 2 | 财报、业绩预告、业绩快报 | 质量、成长、盈利修正 | P2 优先接入 |
| 3 | 融资融券明细 | 杠杆资金和拥挤度 | P2 接入 |
| 4 | 股东户数、质押、限售解禁 | 供给压力和风险过滤 | P2 接入 |
| 5 | 北向资金 | 资金流状态 | P3 接入 |
| 6 | 龙虎榜、大宗交易 | 事件风险和情绪 | P3 接入 |
| 7 | 公告、新闻、NLP | 潜在高价值但稳定性低 | P4 研究 |
...
1. `D:\data\strategy\quant_strategy_plan.md`：短总纲，记录方向、证据、优先级和禁用结论。
2. `D:\data\strategy\quant_strategy_research_plan_detailed.md`：详细执行规范，记录数据口径、标签、验证、回测、台账、验收和 review 清单。
