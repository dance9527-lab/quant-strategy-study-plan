...
### 验证框架参数调整（Round 2 最终裁决）
- **embargo**：5日 → **10日**（三方共识，基于A股因子自相关实证数据）
- **purge**：max(horizon,20) → **max(horizon*3,40)**
- **训练窗口**：保持**5年**（Review坚持，3年样本不足）
- **OOT steps**：最少**12步 + 分年度分析**（2:1裁决，分年度提供等效市场状态覆盖）
- **S1门槛放宽**：IC t-stat **>1.65** 或 bootstrap p **<0.10**（满足其一，折中于1.5和2.0）
- **弱alpha探索性跟踪**：IC方向一致+分年度正率>70%+成本后不为负 → 记录但不进入keep决策
