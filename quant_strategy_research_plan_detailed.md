...
|---|---|---|---|---|---|---|
| `S1-M` | 月选股正式 alpha 强基线 | 20 日 forward excess/rank，10 日作稳健性 | T 日盘后信号，T+1 执行 | 固定月末或月初首个交易日调仓，持有到下一次月度再平衡；21 日滚动为敏感性 | 63 个交易日；月度重训只作预注册敏感性 | R7 `features/labels` 已支持，仍需 calendar、holdout、测试族台账和实验登记 |
| `S1-D/S1-R` | 日频风险/执行主线，非 alpha keep 主线 | 1 日和 5 日 forward excess/rank，10/20 日作稳健性或交叉对照 | 每个交易日盘后重算 candidate score、risk signal、alert state、GMSL shock state 和执行审计，T+1 执行仅用于离线订单审计 | 默认不主动调仓；离线组合模拟不进入 official keep；tighten-only 生产规则需独立 walk-forward 验证 | 63 个交易日；每日重训只作后置敏感性 | R7 `features/labels` 已支持，仍需 daily profile、orders audit、turnover/capacity、execution label audit 和 GMSL 报告 |
...
正式报告应引用 `warehouse_build_manifest.json` 的 hash，而不是重新手工抄写上表行数。R7 已生成 `features/market_daily_v1` 与 `labels/forward_returns_v1`，各 15,420,654 行，并输出 `feature_label_panel_v1_manifest.json`、PIT feature audit 和 label audit；标签已覆盖 1/5/10/20 日，可支撑 S1-M 月选股正式主线和 S1-D/S1-R 日频风险/执行验证。R11 已初始化 `global_macro_daily`、`gmsl_shock_state` 和 `geopolitical_event_calendar` 三张 GMSL candidate 表，并把它们纳入 leakage check；当前 Cboe VIX/OVX/GVZ 已形成候选行，但 FRED 能源/利率/FX/全球股指仍超时或待接入，GMSL 不能作为 alpha 输入。R7/R11 只解除面板和候选目录阻塞，S1 训练前仍必须引用对应 track 的 walk-forward calendar、holdout log、benchmark audit、validation hash、execution label audit 和实验登记。临时实验缓存只能作为中间产物，不得直接作为官方 S1 evidence。
...
S1-D/S1-R 日频风险/执行当前只能使用 R7 已审计的 1 日和 5 日 forward adjusted-return 标签做 IC、排序、风险预警和离线执行诊断。若研究 open-to-open、close-to-open、overnight、intraday、盘中止损或集合竞价成交标签，必须新建 label set，登记 source/available_at/decision_time，并重跑 PIT feature audit、label audit 和 manifest/hash；未入仓前不得进入 official keep，也不得触发主动调仓或风控动作。
...
### 6.1.5 R5/R11 训练权重、CSRP 和 GMSL

R5/R10/R11 后，concept shift 的近期执行方案不再采用双轨、动态 alpha 或在线 Track B，而是在 S1-M 正式主线的单轨 5 年 rolling 框架内加入预注册样本权重；S1-D/S1-R 以同样隔离规则做正式日频风险/执行报告，但不是 alpha keep 主线：
...
- R10 动态 IC 换手公式只能作为 Phase 2 report-only 或 tighten-only 诊断；输入必须是至少滞后一 step 的成熟 trailing IC。验证前有效上限必须写成 `min(0.10, formula)`，trailing IC 变好也不得 loosen 到 15%，只能报告或收紧。
...
- R5/R10/R11/CSRP/GMSL 指数衰减权重登记：等权对照、row-equal/date-balanced、12 月默认、18 月预注册敏感性、6/24 月诊断、63 日重训锚点、成熟 IC 状态机、结构性 regime map、GMSL shock-state、拥挤容量和参数 hash。
...
- S1.5 CSRP/GMSL：等权 5 年、row-equal/date-balanced 12 月半衰期、18 月半衰期、R10 6/24 月诊断、可选 36 月研究网格、4/6 年训练窗口诊断、结构性 regime map、GMSL shock-state、拥挤容量、forced deleveraging 和 63 日重训频率对照。6/24/36 月网格、4/6 年窗口和 GMSL 阈值只作研究记录，计入 `attempt_count`，不得进入 keep/晋级，不得替代 5 年单轨主证据。
- 风险开关报告。
- 回撤改善报告。
