...
S1 阶段同时维护两条同等重要的预注册主线。它们是不同持有周期和调仓频率的研究 cadence，不是 R5 后已废弃的 Track A/B、自适应 alpha 或在线微调。

| track_id | 名称 | 主标签 | 决策和执行 | 调仓/持有 | 默认重训 | 当前数据状态 |
|---|---|---|---|---|---|---|
| `S1-M` | 月选股强基线 | 20 日 forward excess/rank，10 日作稳健性 | T 日盘后信号，T+1 执行 | 约 21 个交易日调仓，持有到下一次月度再平衡 | 63 个交易日；月度重训只作预注册敏感性 | R7 `features/labels` 已支持，仍需 calendar、holdout、测试族台账和实验登记 |
| `S1-D` | 日选股强基线 | 1 日和 5 日 forward excess/rank，10/20 日作稳健性或交叉对照 | 每个交易日盘后重算分数，T+1 执行 | 默认每日滚动调仓或分批更新；不做 T+0 | 63 个交易日；每日重训只作后置敏感性 | R7 `features/labels` 已支持，仍需 daily profile、orders audit、turnover/capacity 报告 |

每条主线必须独立记录 `track_id`、`label_id`、`calendar_id`、`panel_hash`、`execution_rule_id`、`test_family_id`、`attempt_count` 和 holdout 访问状态。不得用某条主线的 OOT、holdout 或 stress slice 结果决定另一条主线是否 keep、晋级、换参数或停用。
...
正式报告应引用 `warehouse_build_manifest.json` 的 hash，而不是重新手工抄写上表行数。R7 已生成 `features/market_daily_v1` 与 `labels/forward_returns_v1`，各 15,420,654 行，并输出 `feature_label_panel_v1_manifest.json`、PIT feature audit 和 label audit；标签已覆盖 1/5/10/20 日，可同时支撑 S1-D 日选股和 S1-M 月选股。R7 只解除面板占位阻塞，S1 训练前仍必须引用对应 track 的 walk-forward calendar、holdout log、benchmark audit、validation hash 和实验登记。临时实验缓存只能作为中间产物，不得直接作为官方 S1 evidence。
...
| 标签 | 计算 | 用途 | 必须 purge |
|---|---|---|---|
| `excess_ret_1d` | 个股 T+1 收益减 benchmark | S1-D 日选股主标签之一；同时可作快速 IC | 1 个交易日 |
| `excess_ret_5d` | 个股未来 5 日超额收益 | S1-D 日选股主标签之一，用于降低 1 日噪声和换手敏感性 | 5 个交易日 |
| `excess_ret_10d` | 个股未来 10 日超额收益 | qant 对照和中频模型 | 10 个交易日 |
| `excess_ret_20d` | 个股未来 20 日超额收益 | S1-M 月选股主标签 | 20 个交易日 |
| `rank_ret_1d/5d/20d` | 同日横截面未来收益 rank | S1-D 和 S1-M 排序模型 | 对应 horizon |
| `top_quantile_1d/5d/20d` | 是否进入未来收益 top 分位 | S1-D 和 S1-M 分类模型 | 对应 horizon |
| `realized_vol_20d` | 未来 20 日波动率 | 风险模型 | 20 个交易日 |
| `max_dd_20d` | 买入后 20 日最大回撤 | 风控标签 | 20 个交易日 |

标签从 `return_adjusted_pit` 计算，成交约束从未复权价格和可交易表计算。

S1-D 日选股当前只能使用 R7 已审计的 1 日和 5 日 forward adjusted-return 标签。若研究 open-to-open、close-to-open、overnight、intraday、盘中止损或集合竞价成交标签，必须新建 label set，登记 source/available_at/decision_time，并重跑 PIT feature audit、label audit 和 manifest/hash；未入仓前不得进入 official keep。
...
R5 后，concept shift 的近期执行方案不再采用双轨、动态 alpha 或在线 Track B，而是在 S1-M 和 S1-D 各自的单轨 5 年 rolling 框架内加入预注册样本权重：
...
| 主数据起点 | 2005-01-01 |
| 训练窗口 | 5 年 |
| track profile | `S1-M` 月选股和 `S1-D` 日选股平行启动、独立登记 |
| S1-M 测试/调仓窗口 | 20 日标签，约 21 个交易日调仓/预测 |
| S1-D 测试/调仓窗口 | 1/5 日标签，每日盘后预测 T+1 候选；可按日度滚动调仓或预注册分批更新 |
| 训练权重 | 等权 5 年必须作为对照；R5 指数衰减候选默认半衰期 12 个月，18 个月为预注册敏感性；row-equal 与 date-balanced 均需报告 |
...
- 组合回测必须处理重叠持仓、T+1、100 股整数手、涨跌停/停牌订单失败、未成交继续暴露和每日换手。
...
- Concept shift 诊断和 R5 训练权重必须分层：
  ...
  - S1.5 只评估预注册指数衰减半衰期、权重口径和重训频率敏感性，不评估双轨、动态 alpha 或在线 Track B。
...
|---|---|---|---|---|
| `S1-M` | 月选股 | 20 日 forward excess/rank | 约 21 个交易日调仓，低换手持有到下次再平衡 | 月末集中成交、持仓拥挤、长持有期回撤 |
| `S1-D` | 日选股 | 1 日和 5 日 forward excess/rank | 每日盘后候选，T+1 执行，日度滚动调仓或分批更新 | 高换手、成本拖累、涨跌停/停牌成交失败、短期噪声 |

两条主线同等重视、平行启动，但必须各自有独立 `track_id`、calendar、label、orders audit、capacity report 和实验台账。不得把一条主线的 OOT/holdout 结果作为另一条主线的选择依据。
...
| track registry | 固化 `S1-M` 和 `S1-D` 的 `track_id`、label、decision frequency、rebalance、holding period、execution rule 和 primary metric | P0，阻塞双主线正式实验登记 |
...
| S1-D 日选股执行审计 | 预注册 `daily_selection_rebalance_v1`、`daily_orders_audit`、`daily_turnover_capacity_report` 和成本 1x/2x/3x 敏感性 | P1，阻塞日选股 keep 和容量结论 |
...
- S1-D 必须单独报告日换手、订单失败、短持有期滚动持仓会计、成本 1x/2x/3x 敏感性和与 S1-M 的持仓重叠度。月选股通过 1 亿容量不代表日选股也通过。
...
目标：在 S1-M 或 S1-D 各自的单轨强基线已经完成、可复现并通过主审计后，评估低复杂度时间衰减权重、结构性 regime 稳健性和拥挤容量是否能降低回撤、左尾和执行失败。S1.5 不是替代训练轨，不改变当前 step 预测或仓位，不替代任一主线的 S1 主证据，也不阻塞 S1 启动；但它阻塞“可部署策略”或“生产准备完成”的叙事。
...
- Refit frequency：默认每 63 个交易日重训一次；S1-M 约 21 个交易日调仓/预测，S1-D 每日预测/调仓候选但不默认每日重训；月度重训或每日重训只能作为预注册敏感性。
...
- `track_registry_v1`：登记 `S1-M` 与 `S1-D` 的标签、决策频率、调仓间隔、持有期、执行规则、容量规则和主指标。
...
- S1-D 面板绑定：1/5 日标签、日度 decision calendar、daily candidate schema、日度订单审计规则和对应 hash。

### Step 2：单因子和分层验证

输出：

- S1-M 与 S1-D 分别输出 IC / RankIC / ICIR，不得混成一个 S1 指标。
...
- 因子相关性。
- 中性化前后对比。
- S1-D 额外输出：日度候选数量、日度行业/市值暴露、与 S1-M 候选重叠度、冲突买卖统计和短期反转风险。
...
- `S1-D_daily_selection_baseline`：1/5 日标签、每日候选、日度滚动组合、等权、ICIR、线性基线；非线性模型只作后续对照。
- 成本后收益，按 S1-M 和 S1-D 分开报告。
- 成交失败、连续锁死和解锁后反转，按 S1-M 和 S1-D 分开报告。
- S1 最小容量：1000 万、5000 万、1 亿；月选股通过的容量不得自动转给日选股。
- `orders_audit` 或等价订单状态机产物；S1-D 必须有 `daily_orders_audit`。
- 拥挤容量、limit-lock CVaR 和成交额占比；S1-D 必须额外报告日换手、成本 1x/2x/3x、日度参与率和未成交继续暴露。
- 资金规模敏感性。
...
- S1.5 CSRP：等权 5 年、row-equal/date-balanced 12 月半衰期、18 月半衰期、可选 6/24/36 月研究网格、结构性 regime map、拥挤容量和 63 日重训频率对照。6/24/36 月网格只作研究记录，计入 `attempt_count`，不得进入 keep/晋级，不得替代 5 年单轨主证据。
- 风险开关报告。
- 回撤改善报告。
