...
| walk-forward 日历生成 | 固化主窗口起点、首个 OOT 起点、holdout 起点、全量 OOT step 数、每步训练截止日和 `walk_forward_calendar_hash` | P0，阻塞 S1 设计 |
...
目标：在 S1 单轨强基线已经完成、可复现并通过主审计后，评估低复杂度时间衰减权重、结构性 regime 稳健性和拥挤容量是否能降低回撤、左尾和执行失败。S1.5 不是替代训练轨，不改变当前 step 预测或仓位，不替代 S1 主证据，也不阻塞 S1 启动；但它阻塞“可部署策略”或“生产准备完成”的叙事。
...
- `orders_audit` 或等价订单状态机产物。
- 拥挤容量、limit-lock CVaR 和成交额占比。
- 资金规模敏感性。
